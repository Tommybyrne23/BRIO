...
Next.js 16 (App Router) decision-led BRIO PWA, with PostgreSQL via Drizzle ORM and authentication via Better Auth. The primary areas are Today, Training, Nutrition, Recovery, and Data; `/demo` is a deterministic labelled guest path.
...
| `npm run build`                                                        | Production standalone build. The Docker runner copies standalone output, static chunks, and `public`; use the release runbook to reproduce that layout locally rather than `next start`.                                                                          |
| `npm run lint`                                                        | ESLint                                                                                                                                                                                                                                                            |
| `npm run test:contracts`                                               | Shared contract and safety invariant tests.                                                                                                                                                                                                                        |
| `npm run test:demo`                                                    | Deterministic fixture, five-action, consent, and demo mutation tests.                                                                                                                                                                                               |
...
`POST /api/health-samples` accepts `{ samples: [...] }` (or a bare array/single object), each row requiring `sampleType`, `value`, `startDate`, `endDate`, with optional `unit`, `sourceName`, `metadata`, and `externalId`. Stable IDs are unique per owner, not globally. `/api/health-samples/deletions` reconciles HealthKit deletions for the authenticated owner only.
...
Required env vars on the Dokploy app: `DATABASE_URL`, `BETTER_AUTH_SECRET`, and `BETTER_AUTH_URL`. `BETTER_AUTH_URL` must be the **exact public HTTPS origin** or browser writes fail Better Auth's origin check. `OPENAI_API_KEY` is optional for the deterministic product/demo but required for live agent/worker execution. Optional: `OPENAI_AGENTS_MODEL` (defaults to `gpt-5-mini`) and `WORKER_INTERVAL_MINUTES` (defaults to `1440`, i.e. daily — worker only). The worker skips accounts without explicit server-AI processing consent.

The full release/deployment procedure, verified commands, mobile flow, rollback, known gaps, and demo path are in `../../docs/manus/runbook.md`.
